...
@section('form')
    <form id="contactForm" action="{{route('document.update', $document->id)}}" method="post">
        @csrf
        @method('put')
        <div class="mb-3">
            <label class="form-label" for="level">Nama Surat</label>
            <input class="form-control" id="level" type="text" value="{{$document->name}}" name="document_name" placeholder="Masukan Nama Surat"
                   data-sb-validations="required" required/>
            <div class="invalid-feedback" data-sb-feedback="level:required">Level is required.</div>
        </div>
        <div id="requirements">
            @foreach($document->requirements as $index => $requirement)
                <div class="mb-3">
                    <label class="form-label" for="jabatan">Syarat {{$index+1}}</label>
                    <div class="row">
                        <div class="col-11">
                            <input class="form-control" id="jabatan" value="{{$requirement->name}}" type="text" name="requirement[]" placeholder="Masukan Nama Syarat" data-sb-validations="required" />
                        </div>
                        <div class="col-1">
...
@push('scripts')
    <script>
        let COUNTER = {{count($document->requirements)}};
