...
@section('form')
    <div class="d-flex mb-3">
        <img id="cover-preview" class="align-content-center" width="50%" src="/storage/{{$staff->image}}" alt="{{$staff->name}}">
    </div>
    ...
    <form id="contactForm" action="{{route('staff.update', $staff->id)}}" method="post">
        @csrf
        @method('put')
        <input type="hidden" name="image" id="image" value="{{$staff->image}}">
        <div class="mb-3">
            <label class="form-label" for="nama">Nama</label>
            <input class="form-control" id="nama" type="text" name="name" placeholder="Nama"
                   data-sb-validations="required" value="{{$staff->name}}"/>
